"""Error research node — searches the web to help resolve FLAML execution errors.

When the FLAML agent encounters an error during code execution, this node
...
async def error_research(state: dict) -> dict:
    ...
    query = (
        f"python FLAML AutoML error fix: {error_snippet}\n"
        "How to fix this error in a FLAML AutoML training script?"
    )

    search_results = ""
    try:
        search_results = await search_with_gemini(query, model=AGENT_MODELS["flaml"])
    ...
    return {
        ...
        "messages": [HumanMessage(content="Searched web for FLAML error resolution.")],
    }
