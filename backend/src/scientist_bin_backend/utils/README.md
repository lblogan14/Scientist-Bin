...
- `artifacts.py` — `save_experiment_artifacts()` copies the trained model (`.joblib`), results JSON, and decision journal from run directories to the top-level `outputs/` directories. Used by both the CLI and the API route after training completes.
